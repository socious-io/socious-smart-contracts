...
describe('Donate', async () => {
  async function donateSetup() {
    ...
    await donateContract.addToken(mockUSDCContract.address)
  ...
  describe('Check functionalities from Donate using mockUSDC', async () => {
    it('Should list the expected token', async () => {
      ...
      expect((await donateContract.getTokens())[0]).to.equal(mockUSDCContract.address)
    })
    ...
    it('Should donate and update the balances from owner, sender and organization', async () => {
      ...
      await expect(
        await senderDonate.donate('123', reciever.address, parseUnits('1.0', 'ether'), mockUSDCContract.address)
      )
