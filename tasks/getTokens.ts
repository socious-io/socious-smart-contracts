...
task('getToken', 'Show address of token from target contract')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      contract = await hre.ethers.getContractAt(contractName, fileName.contractName)
      const connectedContract = contract.connect(provider)
      const tokenAddresses = await connectedContract.getTokens()

      const tokenContract = await hre.ethers.getContractAt('ERC20', tokenAddresses[0])
